...
public class Waits {
	...
	public  void untilJqueryIsDone() {
		untilJqueryIsDone(TestContext.getDriver().manage().timeouts().getImplicitWaitTimeout().getSeconds());
	}

	public  void untilJqueryIsDone(Long timeoutInSeconds) {
		until(TestContext.getDriver(), (d) -> {
			Boolean isJqueryCallDone = (Boolean) ((JavascriptExecutor) TestContext.getDriver()).executeScript("return jQuery.active==0");
			if (!isJqueryCallDone)
	...
	public static void untilPageLoadComplete() {
		untilPageLoadComplete(TestContext.getDriver(),
				TestContext.getDriver().manage().timeouts().getImplicitWaitTimeout().getSeconds());
	}
	...
	public static void until(WebDriver driver, Function<WebDriver, Boolean> waitCondition) {
		until(driver, waitCondition, TestContext.getDriver().manage().timeouts().getImplicitWaitTimeout().getSeconds());
	}
